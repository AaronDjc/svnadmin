<%@ page contentType="text/html;charset=UTF-8"%>
<%@page import="org.svnadmin.Constants"%>
<%@page import="org.svnadmin.util.I18N"%>
<%@include file="header.jsp"%>


<div class="outer">
	<div class="inner">
		<ul class="breadcrumb">
             <li>
             <a href="<%=ctx%>/"><i class="font-home"></i>Home</a>
             <span class="divider">/</span> 
             <a href="<%=ctx%>/project"><%=I18N.getLbl(request,"pj.title","项目") %></a>
             <span class="divider">/</span> 
             <a href="<%=ctx%>/project"><%=request.getParameter("pj")%></a>
             <span class="divider">/</span> 
             </li>
             <li class="active">用户组</li>
        </ul>
        <div class="body">
        	<div class="tabbable">
        		<ul class="nav nav-tabs">
                     <li class="active"><a href="#tab1" data-toggle="tab">用户组管理</a></li>
                     <li><a href="#tab2" data-toggle="tab">Section 2</a></li>
                </ul>
                <div class="tab-content">
                    <div class="tab-pane active row-fluid" id="tab1">
                    	<div class="span6 well">
	                    <table class="table table-striped">
							<thead>
								<th>#</th>
								<th><%=I18N.getLbl(request,"pj.pj","项目") %></th>
								<th><%=I18N.getLbl(request,"pj_gr.des","描述") %></th>
								<th><%=I18N.getLbl(request,"pjgr.op.setuser","设置用户") %></th>
								<th><%=I18N.getLbl(request,"pjgr.op.delete","删除") %></th>
							</thead>
							<tbody>
							<%
							java.util.List<org.svnadmin.entity.PjGr> list = (java.util.List<org.svnadmin.entity.PjGr>)request.getAttribute("list");
						
							if(list!=null){
							  for(int i = 0;i<list.size();i++){
								  org.svnadmin.entity.PjGr pjGr = list.get(i);
								%>
								<tr>
								<td><%=(i+1) %></td>								
								<td>
									<%if((pjGr.getPj()+"_"+Constants.GROUP_MANAGER).equals(pjGr.getGr()) || Constants.GROUP_MANAGER.equals(pjGr.getGr())){%><%=pjGr.getGr() %><%}else{%>
									<a href="<%=ctx%>/pjgr?act=get&pj=<%=pjGr.getPj()%>&gr=<%=pjGr.getGr()%>"><%=pjGr.getGr() %></a>
									<%}%>
								</td>
								<td><%=pjGr.getDes() %></td>
								<td><a href="<%=ctx%>/pjgrusr?pj=<%=pjGr.getPj()%>&gr=<%=pjGr.getGr()%>"><%=I18N.getLbl(request,"pjgr.op.setuser","设置用户") %></a></td>
								<td>
									<%if((pjGr.getPj()+"_"+Constants.GROUP_MANAGER).equals(pjGr.getGr()) || Constants.GROUP_MANAGER.equals(pjGr.getGr())){%>&nbsp;<%}else{%>
									<a href="javascript:if(confirm('<%=I18N.getLbl(request,"pjgr.op.delete.confirm","确认删除?") %>')){del('<%=ctx%>/pjgr?pj=<%=pjGr.getPj()%>&gr=<%=pjGr.getGr()%>')}"><%=I18N.getLbl(request,"pjgr.op.delete","删除") %></a>
									<%}%>
								</td>
							</tr>
								<%	
							}}
							%>
							</tbody>
						</table>
	                    </div>
	                    <div class="span6 well">
		                    <%
							org.svnadmin.entity.PjGr entity = (org.svnadmin.entity.PjGr)request.getAttribute("entity");
							if(entity==null)entity=new org.svnadmin.entity.PjGr();
							%>
							<form class="form-horizontal" name="pjgr" action="<%=ctx%>/pjgr" method="post" onsubmit="return checkForm(this);">
								<fieldset>
									<div class="control-group">
										<label class="control-label"><%=I18N.getLbl(request,"pj.pj","项目") %>:</label>
										<div class="controls">
											<input type="hidden" name="pj" value="<%=request.getParameter("pj")%>">
											<%=request.getParameter("pj")%>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label"><%=I18N.getLbl(request,"pj_gr.gr","组号") %>:<span class="req">*</span></label>
										<div class="controls">
											<input class="span12" type="text" name="gr" value="<%=entity.getGr()==null?"":entity.getGr()%>"  onkeyup="value=value.replace(/[^_\-A-Za-z0-9]/g,'')">
										</div>
									</div>
									<div class="control-group">
										<label class="control-label"><%=I18N.getLbl(request,"pj_gr.des","描述") %>:<span class="req">*</span></label>
										<div class="controls">
											<textarea rows="5" cols="5" name="des" class="span12"><%=entity.getDes()==null?"":entity.getDes()%></textarea>
										</div>
									</div>	
									<div class="form-actions">
										<input type="hidden" name="act" value="save">
										<button type="submit" class="btn btn-primary"><%=I18N.getLbl(request,"pjgr.btn.submit","提交") %></button>
									</div>
								</fieldset>
							</form>
	                    </div>
                    </div>
                    <div class="tab-pane" id="tab2">Howdy, I'm in Section 2.</div>
                </div>
        	</div>
	        
			
        </div>
	</div>
</div>
<script>
function checkForm(f){
	if(f.elements["pj"].value==""){
		alert("<%=I18N.getLbl(request,"pjgr.error.pj","项目不可以为空") %>");
		f.elements["pj"].focus();
		return false;
	}
	if(f.elements["gr"].value==""){
		alert("<%=I18N.getLbl(request,"pjgr.error.gr","组号不可以为空") %>");
		f.elements["gr"].focus();
		return false;
	}
	return true;
}
</script>


