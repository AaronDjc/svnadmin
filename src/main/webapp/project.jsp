	        	...
	        	<div class="navbar-inner">
	            	...
	            	<div class="nav pull-right">
                               ...
                               <ul class="dropdown-menu pull-right">
                                   <%if(hasAdminRight){ %>
                                   <li><a href="<%=ctx%>/new"><i class="font-plus"></i>添加项目</a></li>
                                   <%} %>
			...
			<table class="table table-striped">
				...
				if(list!=null){
				  for(int i = 0;i<list.size();i++){
					...
					<td>
						<%if(hasAdminRight){ %>
						<a href="<%=ctx%>/treelist?pj=<%=pj.getPj() %>"><%=pj.getPj() %></a>
						<%}else{%>  
						<%=pj.getPj() %>
						<%}%>  
					</td>
					...
					<td>	
						...
						<li><a class="btn" title="删除" href="javascript:if(confirm('确认删除?')){del('<%=ctx%>/project?pj=<%=pj.getPj()%>')}"><b class="font-minus-sign"></b></a></li>
						<%} %>
						</ul>
					</td>
				</tr>
