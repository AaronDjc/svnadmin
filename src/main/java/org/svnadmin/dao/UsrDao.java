...
public class UsrDao extends Dao {
	...
	public PagedList<Usr> getPagedList(int pageindex,int pagesize) {
		String sql = "select usr,psw,role from usr where usr<>'*' order by usr limit ?,?;";
		List<Usr> list = new ArrayList<Usr>();
		int total = getCount();
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			//return list;
			return new PagedList<Usr>(total,pageindex,pagesize,list);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		} finally {
	...
	public int updatePwd(Usr usr) {
		String sql = "update usr set psw=? where usr=?";
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = this.getConnection();
