...
public class PjDao extends Dao {
	...
	public Pj get(String pj) {
		String sql = "select pj,path,url,des,type,create_datetime,update_datetime from pj where pj = ?";
	...
	public List<Pj> getList() {
		String sql = "select pj,path,url,des,type,create_datetime,update_datetime from pj order by pj";
	...
	private int getCount(){
		String sql = "select count(1) from pj;";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
	...
	public PagedList<Pj> getList(int pageindex,int pagesize) {
		String sql = "select pj,path,url,des,type,create_datetime,update_datetime from pj order by pj limit ?,?;";
		int total = getCount();
		List<Pj> list = new ArrayList<Pj>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);
			int index = 1;
			pstmt.setInt(index++, (pageindex-1)*pagesize);
			...
			//return list;
			return new PagedList<Pj>(total,pageindex,pagesize,list);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		} finally {
	...
	public List<Pj> getList(String usr) {
		String sql = "select p.pj,p.path,p.url,p.des,p.type,pm.pj manager,p.create_datetime,p.update_datetime from ( "
				+ " select distinct a.pj,a.path,a.url,a.des,a.type,a.create_datetime,a.update_datetime from pj a where  "
	...
	public int getCount(String usr){
	...
	public PagedList<Pj> getList(String usr,int pageindex,int pagesize) {
		String sql = "select p.pj,p.path,p.url,p.des,p.type,pm.pj manager,p.create_datetime,p.update_datetime from ( "
				+ " select distinct a.pj,a.path,a.url,a.des,a.type,a.create_datetime,a.update_datetime from pj a where  "
				+ " exists (select b.usr from pj_gr_usr b where a.pj=b.pj and b.usr=?)  "
				+ " or exists(select c.usr from pj_usr_auth c where a.pj=c.pj and c.usr=?) "
				...
				// TODO like ? 应该是 = ? ,用like主要是兼容3.0版本 see: Issue 4
				+ " exists (select b.usr from pj_gr_usr b where a.pj=b.pj and b.usr=? and b.gr like ?)"
				+ " ) pm on p.pj=pm.pj order by p.pj limit ?,?";
		List<Pj> list = new ArrayList<Pj>();
		Connection conn = null;
		int total = getCount(usr);
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = this.getConnection();
	...
	public Pj readPj(ResultSet rs) 
		...
		result.setCreateTimeByLong(rs.getLong("create_datetime"));
		result.setUpdateTimeByLong(rs.getLong("update_datetime"));
		return result;
	}
	...
	public int insert(Pj pj) {
		String sql = "insert into pj (pj,path,url,des,type,create_datetime,update_datetime) values (?,?,?,?,?,?,?)";
		...
		try {
			...
			pstmt.setLong(index++, pj.getCreateTime().getTime());
			pstmt.setLong(index++, pj.getUpdateTime().getTime());
	...
	public int update(Pj pj) {
		String sql = "update pj set path=?,url=?,des=?,type=?,update_datetime=? where pj = ?";
