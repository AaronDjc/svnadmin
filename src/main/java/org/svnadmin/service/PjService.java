...
public class PjService {
	 ...
	 * @return 项目列表
	 */
	public PagedList<Pj> list(int pageindex,int pagesize) {
		return pjDao.getList(pageindex,pagesize);
	}
	public List<Pj> list(){
	...
	public PagedList<Pj> list(String usr,int pageindex,int pagesize) {
		return pjDao.getList(usr,pageindex,pagesize);
	}
	 ...
	 */
	@Transactional
	public void save(Pj pj,boolean isInit) throws SVNException {
		// 路径 把\替换为/
		...
		if (!respository.exists() || !respository.isDirectory()) {// 不存在仓库
			RepositoryService.createLocalRepository(respository,isInit);	
			
			//RepositoryService.		
			
		}
		Date timeline = new Date();//.getTime();
		if (insert) {
			// 增加默认的组			
			pj.setCreateTime(timeline);
			pj.setUpdateTime(timeline);
